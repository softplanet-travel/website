(function(){
  ...
  async function createTrip(values){
    ...
    const {data,error}=await window.spClient.from("trips").insert({user_id:current.user.id,title:values.title,country:values.country||null,city:values.city||null,start_date:values.start_date||null,end_date:values.end_date||null}).select().single();if(error)throw error;return data;
  }
